...
interface BrandSwitcherProps {
  ...
  label: string;
  /** `dict.nav.comingSoon` — the badge on a brand with no site yet. */
  soonLabel: string;
}
 ...
 */
export function BrandSwitcher({ locale, label, soonLabel }: BrandSwitcherProps) {
    ...
    >
      ...
      */}
      <button
        ...
        className={`group -mx-2 flex cursor-pointer items-center gap-2.5 rounded-xl px-2 py-1.5 transition-colors duration-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sea lg:gap-3 ${
          open ? "bg-sea/10" : "hover:bg-sea/8"
        }`}
      >
        <Image
          src="/logo.png"
  ...
  return (
    ...
    >
      {Array.from({ length: 9 }, (_, index) => (
        <span key={index} className="size-[3.5px] rounded-full bg-sea" />
      ))}
    </span>
  );
}
